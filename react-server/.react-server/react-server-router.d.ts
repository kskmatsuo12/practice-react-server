declare namespace __react_server_routing__ {
  ...
  type StaticRoutes = "/" | "/about" | "/posts";
  type DynamicRoutes<T0 extends string, T1 extends string> =
    | `/posts/${SafeSlug<T0>}-${SafeSlug<T1>}`
    | `/posts/${SafeSlug<T0>}/${SafeSlug<T1>}`
    | `/posts/${SafeSlug<T0>}`;
  ...
  type RouteImpl<T> =
    ...
    | (T extends `${DynamicRoutes<infer _, infer __>}${Suffix}`
        ? T
        : never);
    ...
    : P extends `[...${infer K}]`
      ...
      : P extends `[[${infer K}]]`
        ...
        : P extends `[${infer K0}]-[${infer K1}]`
          ? { [key in K0]: string } & { [key in K1]: string } & R
          : P extends `[${infer K}]`
            ? { [key in K]: string } & R
            : R;
